...
data_dir = "/home/dbshin/01_WORKS/99_EXT_DATAS/ERA5/raw_datas/monthly-averaged_20_60_110_150_2017-2019_rad.grib"
...
for tt, ti in enumerate(range(0,ntime_step-1)):
    ncout = nc4.Dataset('{}/sflux_rad_1.{}.nc'.format(outp_dir,str(tt+1).zfill(4)),'w', format='NETCDF4') #'w' stands for write
    ...
    time_nc.units = "days since {0}-{1}-{2} {3}:00".format(sta_year, sta_month, sta_day, sta_hour);
    time_nc.base_date = [sta_year, sta_month, sta_day, sta_hour]
    ...
    tgrb = ti*2
    grb_long = gribfile.GetRasterBand(tgrb+1) 
    grb_short = gribfile.GetRasterBand(tgrb+2) 
    grb_long2 = gribfile.GetRasterBand(tgrb+3) 
    grb_short2 = gribfile.GetRasterBand(tgrb+4) 

    # time_nc[tt]     = time_steps[tt] # hours from start time
    # dlwrf_nc[tt,:,:] = list(reversed(list(grb_long.ReadAsArray())))
    # dswrf_nc[tt,:,:] = list(reversed(list(grb_short.ReadAsArray())))
    ...
    for tti, ttt in enumerate(itime_steps):
        ...
        dlwrf_nc[tti,:,:] = ((len(itime_steps)-tti)*np.array(list(reversed(list(grb_long.ReadAsArray())))) + tti*np.array(list(reversed(list(grb_long2.ReadAsArray())))))/len(itime_steps)
        dswrf_nc[tti,:,:] = ((len(itime_steps)-tti)*np.array(list(reversed(list(grb_short.ReadAsArray())))) + tti*np.array(list(reversed(list(grb_short2.ReadAsArray())))))/len(itime_steps)
    ncout.close()
gribfile  = None
