...
for tt, ti in enumerate(range(0,ntime_step-1)):
    ...
    grb_prate2 = gribfile.GetRasterBand(ti+2)
    ...
    for tti, ttt in enumerate(itime_steps):
        ...
        prate_nc[tti,:,:] = ((len(itime_steps)-tti)*np.array(list(reversed(list(grb_prate.ReadAsArray())))) + tti*np.array(list(reversed(list(grb_prate2.ReadAsArray())))))/len(itime_steps)
    ncout.close()
gribfile  = None
