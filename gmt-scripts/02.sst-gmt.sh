...
do
  ...
  gmt begin ${var_nic}_t-${tt}
    gmt nearneighbor -R${window_x_min}/${window_x_max}/${window_y_min}/${window_y_max} -I${resolution}m -S${fixed_radius}k -G$filename.nc $filename.xyv
  	gmt grdimage $filename.nc -JM10i -Cseafloor -B
    gmt grdcontour $filename.nc -JM10i -C$v_interval -A0.06 -B
    gmt colorbar -DJRM+o1.5c/0+e+mc -By+0.0lm
    gmt coast -R${window_x_min}/${window_x_max}/${window_y_min}/${window_y_max} -W0.1p,black -JM10i -Gdarkseagreen2 -Ba2f0.5g1 -BWSne+t"SST"
  gmt end
done
